...
public class InstructionsPanelImpl extends JComponent implements WizardObserver, Accessible, InstructionsPanel {
    ...
    public final void paintComponent(Graphics g) {
        ...
        Font f1 = getFont() != null ? getFont() : UIManager.getFont("controlFont"); //NOI18N
        Font f = new Font("Calibri", Font.BOLD, f1.getSize()+2);
        ...
        int y = fm.getMaxAscent() + ins.top + MARGIN + 150;
        ...
        int underlineY = ins.top + MARGIN + fm.getAscent() + 3 + 150;
       ...
       // g.setFont (f);
        ...
        for (int i=first; i < stop; i++) {
            ...
            if (curr != null) {
                ...
                //if (steps[i].equals(currentStep) || (i < steps.length-1 && steps[i+1].equals(currentStep))) {
                if (steps[i].equals(currentStep)) {
                	stopDrawing = true;
                }
                if (!stopDrawing) {
                	try {
                		BufferedImage img1 = ImageIO.read(InstructionsPanelImpl.class.getResourceAsStream(
                                "tick_green.png"));
                    	g.drawImage(img1, x, y-13, null);	
                	}
                	catch(Exception e) {
                ...
                if (steps[i].equals (currentStep)) {
                	  g.setFont (f.deriveFont(Font.BOLD));
                	  final Color cyan = new Color(180, 213, 240);
